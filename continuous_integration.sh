...
xcodebuild clean build -configuration "Debug" -sdk iphoneos CODE_SIGN_IDENTITY="$CODE_SIGNING_IDENTITY" PROVISIONING_PROFILE="$PROVISIONING_PROFILE" &> "$build_dir/build_device_debug.log"
if [ $? -ne "0" ]; then
    echo "[STATUS] Build failed"
    echo ""
    exit 1  
